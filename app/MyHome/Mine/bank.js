...
export default bankCardAttribution = (bankCardNum, callback) => {
    ...
    axios.post(`https://ccdcapi.alipay.com/validateAndCacheCardInfo.json?_input_charset=utf-8&cardNo=${bankCardNum}&cardBinCheck=true`, {

    })
        .then((response)=> {
            console.log(response);
            bankType.validated = response.data && response.data.validated;
            if(response.data && response.data.validated){
                let bank = response.data.bank;
                bankType.type = a[bank];
                callback(bankType);
            } else {
                callback(bankType);
            }
